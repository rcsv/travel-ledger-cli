...
### Proposal Fragment `update_estimate`（Planned Money）

Itinerary target のみ。**v4.7.47** で `fragment apply --dry-run` による既存 Estimate 部分更新 preview を実装。DB は read-only で、`--confirm` は未対応（v4.7.48 候補）。

- Estimate identity: `candidate_content.estimate_id`（DB PK）+ 親 Itinerary 検証
- 部分更新: `amount` / `currency`（同時のみ currency 変更可）/ `title` / `note` / `sort_order` / `clear_title` / `clear_note`
- numeric `amount` は minor units；dry-run は DB read-only
- numeric `itinerary_reference` は JSON number（例: `2`）。string `"2"` は既存 resolver 契約により title として扱う

実装正本: [v4.7.47 P-6o update_estimate dry-run](specifications/v4.7.47-p6o-update-estimate-dry-run.md)
