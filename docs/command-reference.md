...
Itinerary target のみ。**v4.7.47** で `fragment apply --dry-run`、**v4.7.48** で `--confirm` による既存 Estimate 部分更新を実装。
...
- numeric `amount` は minor units
- confirm: 単一 transaction、preview agreement、TOCTOU、`WHERE id + itinerary_id`、成功時 `updated_estimate_id`
- no-op（値同一）も confirm success + UPDATE
