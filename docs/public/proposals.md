...
`fragment apply --confirm`: gate 通過後に **add_itinerary**（day target）、**add_note**（trip/day/itinerary）、**add_expense**（itinerary target）、**add_reservation**（itinerary target）を DB へ反映。`--dry-run` と `--confirm` は併用不可（dry-run means no Trip domain data side effects）。
