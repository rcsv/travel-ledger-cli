...
**Estimate（v6+）:** Itinerary 配下の `estimates[]` に Planned Budget（事前見積）を出力します。`amount` / `currency` は必須。v5 export を import する場合、`estimates` キーがなくても **空配列として復元** されます。
...
| `schema_version: 6` | v6 形式（v5 + nested `estimates[]`）。v5 import 能力を包含。**v5 export → v6 import** は `estimates` 省略 = 空 |
...
**v5 Shared Expense 検証:** `paid_by_participant_ref` / `beneficiaries[].participant_ref` が Trip 内 Participant に一意に解決できない export は **`valid: false`** になります。v4 ファイルは v5 専用 ref 検査を **スキップ** します。

**v6 Estimate 検証:** nested `estimates[]` の `amount` / `currency` を検証します。v5 以前のファイルは Estimate 専用検査を **スキップ** します。詳細は [export-schema.md](specifications/export-schema.md) §validate-export。

## 旅行 JSON の比較（trip diff）

2 つの `trip export` JSON を比較し、Trip・Itinerary・Note・Summary・Reservation・**Participant（v4+）**・**Expense payer/beneficiaries（v5+）**・**Estimate（v6+）** の差分を表示します。
...
| Estimate (v6+) | added / removed / field modified（両 export が schema v6+ の場合） |
