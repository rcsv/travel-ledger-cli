...
詳細仕様: **[Reservation モデル](reservation-model.md)**（v1.11.0 — 責務・境界）、**[Entity Design](reservation-entity-design.md)**（v1.12.0 — フィールド）、**[Implementation Plan](reservation-implementation-plan.md)**（v1.13.0 — 実装計画）、**[Responsibilities Review](reservation-responsibilities-review.md)**（v1.19.0 — 実装後レビュー）。
...
- **Entity:** `reservations` テーブル — Itinerary 配下保存、Trip 集約表示（[reservation-responsibilities-review.md](reservation-responsibilities-review.md)）
- **CLI:** `reservation add/list/show/update/delete`
- **Export:** schema v3 `days[].itineraries[].reservations[]`
- **しおり:** `trip export-md` の `## Reservations` セクション

一部の予約情報は canonical sample 等で **Remark** に残る移行期を許容する。

### Future scope（改善候補 — v1.19.0 で文書化）

- `trip doctor` による未予約検出・confirmation 欠落
- 連絡先フィールド、種別固有詳細（flight / rental_car）
- Markdown Itinerary 節内インライン表示
...
| Reservation | `src/reservation.rs` — v1.18.0 実装、[reservation-responsibilities-review.md](reservation-responsibilities-review.md) |
