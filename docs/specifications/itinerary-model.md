...
責務の全体整理: [travel-ledger-responsibilities.md](travel-ledger-responsibilities.md)。**Reservation**（将来）: [reservation-model.md](reservation-model.md)（責務）、[reservation-entity-design.md](reservation-entity-design.md)（フィールド設計）、[reservation-implementation-plan.md](reservation-implementation-plan.md)（実装計画）。Note CRUD の詳細: [note-model.md](note-model.md)。
