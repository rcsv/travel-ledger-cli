...
|---|---|---|---|
| 出発 | Day 1 `06:00` 出発、同行者ピックアップ | `location`: 自宅（example） | `transport` |
...
Expenses:
  - 土産屋さん（個別：Jordan）  SAMPLE-RECEIPT-005  ¥3,700
  - 土産屋さん（個別：Alex）  SAMPLE-RECEIPT-021  ¥8,380
  - SAGAWA BOX（個別：Alex）  SAMPLE-RECEIPT-001  ¥3,200
  - 宅配便（個別：Alex）      SAMPLE-RECEIPT-001  ¥286
```
...
- 金額なし行（例: ロイズ SAMPLE-RECEIPT-033）
...
# 時刻・場所・順序は任意で付与
caglla itinerary add 1 --day 1 --time 06:00 --order 1 --location "自宅（example）" \
  "出発、同行者ピックアップ"
```
...
caglla expense add --itinerary 39 --amount 3700 --currency JPY \
  --title "土産屋さん" --paid-by-name "Jordan" \
  --note "費用区分: 個別：Jordan / 領収書: SAMPLE-RECEIPT-005"
```
...
| **コピーする** | `title`, `note`, `start_time`, `sort_order`, `duration_minutes`, `travel_minutes`, `location`, `category`, Itinerary-level notes（デフォルト）, **Estimate（予定費用）** |
| **コピーしない** | Expense（実績支出）, Reservation（予約実体）, `id`, タイムスタンプ |
