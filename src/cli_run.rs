...
pub fn run() -> Result<()> {
    ...
    let resolved = crate::config::resolve_db_path_for_cli(cli.db.as_deref())?;

    if crate::commands::db::run_before_open_db(&command, &resolved)? {
        return Ok(());
    }
    ...
    match command {
        ...
        Command::Day { action } => match action {
            DayAction::List { trip_id, json } => {
                let detail = crate::services::get_trip_detail(&conn, trip_id)
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
                if json {
                    crate::day::print_day_list_json_from_detail(&detail)?;
                } else {
                    crate::day::print_day_list_display_from_detail(&detail);
                }
            }
            DayAction::Show {
                trip_id,
                ...
                let detail = crate::services::get_day_timeline(&conn, trip_id, day_number)
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
                if json {
                    crate::day::print_day_show_json_from_detail(&detail)?;
                } else {
                    crate::day::print_day_show_display_from_detail(&detail);
                }
            }
            DayAction::Update {
                trip_id,
        ...
        Command::Trip { action } => match action {
            ...
            TripAction::List { json } => {
                let summaries = crate::services::list_trip_summaries(&conn)
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
                if json {
                    let trips: Vec<_> = summaries
                        .iter()
                        .map(crate::trip::trip_summary_to_cli_json)
                        .collect();
                    crate::output::json::print_json(&trips)?;
                } else {
                    crate::trip::print_trip_list_from_summaries(&summaries);
                }
            }
            TripAction::Show { id, json } => {
                let detail = crate::services::get_trip_detail(&conn, id)
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
                if json {
                    crate::output::json::print_json(&crate::trip::trip_detail_to_cli_json(
                        &detail,
                    ))?;
                } else {
                    crate::trip::print_trip_detail_from_detail(&detail);
                }
            }
            TripAction::Update {
                id,
