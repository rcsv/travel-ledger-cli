...
**Itinerary is not a venue.** — Itinerary は場所（Venue / POI）ではなく、**旅行中の行動を表す最小単位** です。計画時には予定として、旅行後には実績として扱えます。`title` と `--day` があれば登録でき、`location`（場所文字列）は **任意** です。高速道路・給油・チェックイン・部屋食・帰宅など、固定 POI に紐づかない行も Itinerary として扱います。費用は Itinerary 自身ではなく、配下の **Expense** に記録します。

詳細: [`docs/specifications/itinerary-model.md`](docs/specifications/itinerary-model.md)
...
- **Itinerary（行動・予定）** の登録・一覧・詳細・更新・削除
- 各 Itinerary への **開始時刻・並び順・所要時間・移動時間・カテゴリ** の設定（**場所 `location` は任意**）
...
Itinerary 配下の **支出記録** です。旅行中に金額を素早く残すことを優先し、`amount` + `currency` のみで追加できます。

Export schema v3 / 精算（Settlement）機能は **未対応**（v1.6.0 で Expense の export/import は対応済み。精算は将来）。
...
cargo run -- expense add --itinerary 12 --amount 980 --currency JPY \
  --title コンビニ --paid-by-name 太郎 --expense-date 2026-04-27 --note メモ
...
cargo run -- expense list --trip 1 --json
cargo run -- expense show 1 --json
```
...
| `expense list` | `--itinerary` または `--trip` のいずれか（Trip 集約一覧は v1 から提供） |
| 金額の保存 | DB は最小通貨単位の **整数**（JPY=円、USD `12.50` → 1250 セント） |
| 通貨コード | 英字 3 文字（小文字入力は大文字に正規化。未知コードも形式が合えば許可） |

Itinerary / Trip 削除時に関連 Expense は自動削除されます（DB トランザクション内）。

詳細仕様: [`docs/specifications/expense-model.md`](docs/specifications/expense-model.md)

### Itinerary

**Itinerary は Day 内の行動単位** です。観光地や POI の一覧ではなく、「いつ・何をするか」を時系列で計画・記録する単位です。`title` と `--day` が必須で、`--location` は任意です（出発・高速道路・部屋食・帰宅など、場所を特定しない行も登録できます）。

Itinerary は **Trip ID** に紐づきます。先に `trip add --start ... --end ...` で旅行を作成してください。

CLI では **`--day N`（何日目か）** で指定します。内部では `days.id`（`day_id`）に解決して保存し、読み取り時は `day_number` を JOIN して返します。export JSON の `day` フィールド（schema v1）は従来どおりです。

**ID の指定について:** `list` 系コマンドでは親リソース（旅行）の ID を指定します。`show` / `update` / `delete` 系では、対象の Itinerary ID を指定します。ID が `1`, `2`, `3` のように小さい整数でも、コマンドごとに意味が異なる点に注意してください。

- `itinerary list 2` … 旅行 ID 2 に属する Itinerary 一覧を表示
- `itinerary show 11` … Itinerary ID 11 の詳細を表示
- Itinerary ID は `itinerary list <trip_id>` の一覧（先頭の `ID` 列）で確認できます

詳細仕様: [`docs/specifications/itinerary-model.md`](docs/specifications/itinerary-model.md)

### Itinerary を追加

```bash
cargo run -- itinerary add 1 --day 1 --time 06:00 --order 1 "出発"
cargo run -- itinerary add 1 --day 1 --time 06:30 --location "東浦→セントレア" \
  "高速道路 東浦→セントレア"
...
cargo run -- itinerary add 1 --day 1 --time 18:00 "夕食 部屋"
...
| `--location` | 場所の自由記述（任意。施設名・区間名・住所など） |
| `--note` | 短い補足メモ（任意） |
...
一覧は **日目 → 並び順（`sort_order`）→ `id`** の順で表示されます。`start_time` は任意の時刻ラベルであり、並び順の主キーではありません。
...
旅行の 1 日の流れを、**並び順（`sort_order`）どおり** に見やすく表示します。各行に `start_time` があれば時刻も表示します。
...
`trip export-md` / `trip stats` の見た目確認用に、4日間・Itinerary 15件・チェックリスト10件のサンプルデータを一括投入できます（観光地中心のデモデータ）。
...
沖縄旅行の 1 日目を登録し、タイムラインで確認する例です（観光・食事・チェックインなど **行動** を `--day` / `--time` / `title` で追加。`--location` は任意）。  
...
| バージョン | 概要 |
|---|---|
| [v1.13.0](docs/releases/v1.13.0-notes.md) | Reservation implementation plan |
| [v1.12.0](docs/releases/v1.12.0-notes.md) | Reservation entity design |
| [v1.11.0](docs/releases/v1.11.0-notes.md) | Reservation model specification |
| [v1.10.0](docs/releases/v1.10.0-notes.md) | Travel Ledger responsibilities |
| [v1.9.0](docs/releases/v1.9.0-notes.md) | Sequence-first ordering unification |
| [v1.8.1](docs/releases/v1.8.1-notes.md) | Travel activity terminology consistency |
| [v1.8.0](docs/releases/v1.8.0-notes.md) | Itinerary model (not a venue) |
| [v1.7.0](docs/releases/v1.7.0-notes.md) | Canonical sample dataset and expense verification |
| [v1.6.0](docs/releases/v1.6.0-notes.md) | Export Schema v3 and Expense backup support |
| [v1.5.0](docs/releases/v1.5.0-notes.md) | Expense CRUD support |
| [v1.4.1](docs/releases/v1.4.1-notes.md) | trip diff Note support |
| [v1.4.0](docs/releases/v1.4.0-notes.md) | Export Schema v2 (Notes) |
