#!/usr/bin/env bash
# trip advisor 全パターンの検証出力を生成する
#
# 使い方（リポジトリルートから）:
#   bash samples/advisor/generate_outputs.sh
#
# 出力先:
#   samples/advisor/outputs/*.txt

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUT_DIR="$ROOT/samples/advisor/outputs"
cd "$ROOT"
...
write_header() {
  ...
  local description="$3"
  {
    echo "# trip advisor validation: $title"
    echo "# Generated: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    echo "#"
    echo "# $description"
    echo "#"
    echo "# Command: cargo run -- trip advisor 1"
    echo ""
  } >"$file"
}
...
write_header "$OUT_DIR/06-missing-duration.txt" "missing duration" \
  "MissingDuration warning + advice"
